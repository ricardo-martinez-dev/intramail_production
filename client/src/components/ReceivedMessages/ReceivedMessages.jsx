...
class ReceivedMessages extends React.Component {
  ...
  fetchReceivedMessages = async () => {
    ...
    const url = `${process.env.REACT_APP_BASE_URL}/api/v1/messages/received`;
  ...
  async componentDidMount() {
    ...
    const intervalId = setInterval(
      async () => this.fetchReceivedMessages(),
      1000
    );
        ...
        <ul id="msg-headers">
          ...
          {!isLoading &&
            messages.map((msg) => {
              ...
              const isRead = msg.is_read ? true : false;
              ...
              return (
                <li
                  ...
                  onClick={() => {
                    onShowUserInfo({ contact: msg.sender });
                    onReadMessage({ msg, action: "received" });
                    onHideNewMessage();
                    onCurrentShown("messageReader");
                    onUpdateReadMessage({
                      id: msg.msg_id,
                      isRead: true,
                    });
